...
LoopExecution::~LoopExecution() {
    if (!mTmpBuffers.empty()) {
        auto pool = static_cast<OpenCLBackend*>(backend())->getBufferPool();
        for (auto& [_, buf] : mTmpBuffers) {
            if (buf) pool->recycle(buf, true);
        }
        mTmpBuffers.clear();
    }
}
...
ErrorCode LoopExecution::onEncode(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs){
    ...
    // Recycle temp buffers to prevent CL_INVALID_MEM_OBJECT on buffer pool reuse
    for (auto& [_, buf] : mTmpBuffers) {
        if (buf) bufferPool->recycle(buf, false);
    }
    mTmpBuffers.clear();
    return NO_ERROR;
}
