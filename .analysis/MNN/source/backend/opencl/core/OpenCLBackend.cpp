...
void OpenCLBackend::setGpuMode(const int cl_mode_num) {
    ...
    // Override: force IMAGE mode on Adreno (BUFFER is 2-3x slower on Adreno 750)
    if (gpuType == ADRENO) {
        mMemType = IMAGE;
    }
    if(totalSet > 1) {
    ...
    // Override: force FAST tuning to avoid OOM/hang on complex models (BV2 flow with WIDE)
    mTuneLevel = Fast;
    totalSet = 0;
